...
export const saveBreak = async (breakData) => {
  ...
  try {
    // Save to Excel first (source-of-truth file for local reports)
    const excelResult = saveToExcel('Breaks', excelData);

    // Ending a break should feel instant in UI, so cap Firebase wait time.
    if (breakData.action === 'end') {
      const firebaseResult = await Promise.race([
        saveToFirebase('breaks', firebaseData),
        new Promise((resolve) => {
          setTimeout(() => resolve({ success: false, timedOut: true }), 900);
        })
      ]);

      if (firebaseResult.timedOut) {
        console.warn('Break end Firebase save timed out (response returned early).');
      }
    ...
    // For break start, keep existing behavior and wait for Firebase acknowledgement.

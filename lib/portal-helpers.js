...
export const assignHalfDayLeaveByManager = async ({
  ...
  await createNotification({
    ...
    title: "Half day leave assigned",
    message: `${managerName || "Manager"} marked ${date} as half day leave.`,
    type: "leave",
    meta: { leaveId: ref.id },
  });
  ...
  await sendPortalEmail({
    to: emp.email,
    subject: `Half Day Leave Applied: ${date}`,
    htmlContent: `<p>Hello ${emp.name}, your manager has marked <b>${date}</b> as <b>Half Day Leave</b>.</p><p>Reason: ${reason}</p>`,
  });
...
export const fetchMonthlyAttendanceSummary = async ({ year, month, employees: sourceEmployees }) => {
  ...
  leaves.forEach((row) => {
    ...
    const isHalfDay = leaveType.includes("half") || Number(row.days) === 0.5 || row.managerAssigned;
    if (!isHalfDay) return;

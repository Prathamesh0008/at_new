...
const mapDoc = (snapshot) => {
  ...
  const appliedAtDate = toDate(data.appliedAt) || toDate(data.createdAt) || toDate(data.managerAssignedAt);
  return {
    id: snapshot.id,
    ...data,
    appliedAtDate,
...
const sortByLatest = (items) =>
  [...items].sort((a, b) => {
    const timeA = a.createdAtDate?.getTime() || a.appliedAtDate?.getTime() || 0;
    const timeB = b.createdAtDate?.getTime() || b.appliedAtDate?.getTime() || 0;
    return timeB - timeA;
  });

const getEmployeeById = (employeeId) => employees.find((emp) => emp.id === employeeId);

const assertManagerAccess = ({ actorId, actorName, action = "perform this action" }) => {
  const actor = getEmployeeById(actorId);
  const role = String(actor?.role || "").trim().toLowerCase();

  if (role !== "manager") {
    const displayName = actorName || actor?.name || "This user";
    throw new Error(`${displayName} is not authorized to ${action}.`);
  }
...
const formatTaskStatusLabel = (status) => {
  ...
  return statusMap[String(status || "").toLowerCase()] || String(status || "Updated");
};
...
export const fetchNotifications = async ({ recipientId, role }) => {
  const docs = await getVisibleNotificationDocs({ recipientId, role, unreadOnly: false });
...
const getVisibleNotificationDocs = async ({ recipientId, role, unreadOnly = false }) => {
  const ref = collection(db, COLLECTIONS.NOTIFICATIONS);
  const unreadFilter = unreadOnly ? [where("read", "==", false)] : [];

  const requests = [
    getDocs(query(ref, where("recipientId", "==", recipientId), ...unreadFilter)),
    getDocs(query(ref, where("recipientId", "==", null), where("recipientRole", "==", null), ...unreadFilter)),
  ];

  if (role) {
    requests.push(getDocs(query(ref, where("recipientRole", "==", role), ...unreadFilter)));
  }
  ...
  snapshots.forEach((snapshot) => {
    snapshot.docs.forEach((docSnap) => {
      if (!docMap.has(docSnap.id)) docMap.set(docSnap.id, docSnap);
    });
  });
...
export const markAllNotificationsRead = async ({ recipientId, role }) => {
  const docs = await getVisibleNotificationDocs({ recipientId, role, unreadOnly: true });
  const targets = docs
...
export const assignHalfDayLeaveByManager = async ({
  ...
  const manager = assertManagerAccess({
    actorId: managerId,
    ...
    approvalTrail: [
      {
        ...
        by: manager.id,
        approverName: manager.name || managerName || "Manager",
        comment: reason,
      },
    ],
    reviewComment: reason,
    managerAssigned: true,
    managerAssignedById: manager.id,
    managerAssignedByName: manager.name || managerName || "Manager",
  ...
  await createNotification({
    ...
    message: `${manager.name || managerName || "Manager"} marked ${date} as ${leaveTypeLabel.toLowerCase()} leave.`,
    type: "leave",
    meta: { leaveId: ref.id },
  });
...
export const reviewLeaveRequest = async ({ leaveId, decision, approverId, approverName, comment = "" }) => {
  ...
  trail[trailIndex] = {
    ...
    at: new Date().toISOString(),
    by: manager.id,
    approverName: manager.name || approverName,
    comment,
  };
  ...
  await createNotification({
    ...
    message: `${manager.name || approverName} marked your leave request as ${nextStatus}.`,
    type: "leave",
    meta: { leaveId },
  });
...
export const updateLeaveReason = async ({ leaveId, employeeId, employeeName, reason }) => {
  ...
  const leave = snap.data();
  if (leave.employeeId !== employeeId) throw new Error("You can edit only your own leave.");
  if (leave.managerAssigned) throw new Error("Manager-assigned leaves cannot be edited.");

  const isEditable = ["pending", "pending_l1", "pending_l2"].includes(String(leave.status || "").toLowerCase());
  if (!isEditable) throw new Error("You can edit reason only before approval or rejection.");
  if (String(leave.reason || "").trim() === nextReason) return;

  await updateDoc(ref, {
    reason: nextReason,
    reasonUpdatedAt: new Date().toISOString(),
    reasonUpdatedBy: employeeId,
    updatedAt: serverTimestamp(),
  });

  await createNotification({
    recipientRole: "manager",
    title: "Leave reason updated",
    message: `${employeeName || leave.employeeName} updated leave reason (${leave.fromDate} to ${leave.toDate}).`,
    type: "leave",
    meta: { leaveId },
  });
};
...
export const cancelManagerAssignedLeave = async ({ leaveId, managerId, managerName, comment = "" }) => {
  const manager = assertManagerAccess({
    actorId: managerId,
    actorName: managerName,
    action: "cancel manager-assigned leave",
  });
  ...
  await updateDoc(ref, {
    ...
    cancelledByName: manager.name || managerName || "Manager",
  ...
  await createNotification({
    ...
    message: `${manager.name || managerName || "Manager"} cancelled your leave (${leave.fromDate} to ${leave.toDate}).`,
    type: "leave",
    meta: { leaveId },
  });
...
export const createTask = async (payload) => {
  ...
  const ref = await addDoc(collection(db, COLLECTIONS.TASKS), {
    ...
    imageDataUrl: primaryImage?.imageDataUrl || payload.imageDataUrl || "",
    imageName: primaryImage?.imageName || payload.imageName || "",
  ...
  const safeAssignerName = escapeHtml(payload.assignedByName || "Manager");
  const safeTaskTitle = escapeHtml(payload.title || "Untitled task");
  const safeTaskDetails = escapeHtml(payload.details || "No task details provided.");
  const safeAssignedTime = escapeHtml(formatTaskDateTime(assignedAtISO));
  const assignmentEmailHtml = `
    <p>${safeAssignerName} assigned a new task.</p>
    <p><b>Task:</b> ${safeTaskTitle}</p>
    <p><b>Details:</b> ${safeTaskDetails}</p>
    <p><b>Assigned Time:</b> ${safeAssignedTime}</p>
  `;
  await sendPortalEmail({
    to: emp?.email,
    subject: `New task assigned: ${payload.title}`,
    htmlContent: assignmentEmailHtml,
  });
...
export const fetchTasks = async (filters = {}) => {
  ...
  const rows = sortByLatest(snapshot.docs.map(mapDoc)).map((row) => {
    const normalizedAssignedTo = row.assignedTo || row.employeeId || "";
    const normalizedAssignedBy = row.assignedBy || row.createdBy || MANAGER.id || "";
    const assignedEmployee = getEmployeeById(normalizedAssignedTo);
    const assignedByEmployee = getEmployeeById(normalizedAssignedBy);

    return {
      ...row,
      assignedTo: normalizedAssignedTo,
      assignedBy: normalizedAssignedBy,
      assignedToName: row.assignedToName || row.employeeName || assignedEmployee?.name || normalizedAssignedTo,
      assignedByName: row.assignedByName || assignedByEmployee?.name || MANAGER.name || normalizedAssignedBy,
      imageDataUrl: row.imageDataUrl || row.imageUrl || "",
...
export const updateTaskStatus = async ({ taskId, status, actorId, actorName }) => {
  ...
  if (status === "in_progress" && !task.startedAt) updates.startedAt = actionAtISO;
  if (status === "completed") updates.completedAt = actionAtISO;
  if (status === "cancelled") updates.cancelledAt = actionAtISO;
  if (status === "assigned") updates.reopenedAt = actionAtISO;
  ...
  const safeActorName = escapeHtml(actorName || task.assignedToName || "Employee");
  const safeTaskTitle = escapeHtml(task.title || "Untitled task");
  const safeTaskDetails = escapeHtml(task.details || "No task details provided.");
  const safeAssignee = escapeHtml(task.assignedToName || task.employeeName || task.assignedTo || "-");
  const safePriority = escapeHtml(task.priority || "medium");
  const prevStatusLabel = formatTaskStatusLabel(previousStatus);
  const nextStatusLabel = formatTaskStatusLabel(status);
  const safePrevStatus = escapeHtml(prevStatusLabel);
  const safeNextStatus = escapeHtml(nextStatusLabel);
  const safeDueDate = escapeHtml(formatTaskDueDate(task.dueDate));
  const startedAtValue = task.startedAt || (status === "in_progress" ? actionAtISO : null);
  const completedAtValue = task.completedAt || (status === "completed" ? actionAtISO : null);
  const safeStartedAt = escapeHtml(formatTaskDateTime(startedAtValue));
  const safeCompletedAt = escapeHtml(formatTaskDateTime(completedAtValue));
  const safeUpdatedAt = escapeHtml(formatTaskDateTime(actionAtISO));
  const assignee = getEmployeeById(task.assignedTo);
  const recipientEmails = Array.from(new Set([MANAGER.email, assignee?.email].filter(Boolean)));
  const statusUpdateHtml = `
      <p><b>${safeActorName}</b> updated task status.</p>
      <p><b>Task:</b> ${safeTaskTitle}</p>
      <p><b>Details:</b> ${safeTaskDetails}</p>
      <p><b>Assigned To:</b> ${safeAssignee}</p>
      <p><b>Priority:</b> ${safePriority}</p>
      <p><b>Due Date:</b> ${safeDueDate}</p>
      <p><b>Started Time:</b> ${safeStartedAt}</p>
      <p><b>Completed Time:</b> ${safeCompletedAt}</p>
      <p><b>Updated Time:</b> ${safeUpdatedAt}</p>
      <p><b>Status:</b> ${safePrevStatus} -> <b>${safeNextStatus}</b></p>
    `;
